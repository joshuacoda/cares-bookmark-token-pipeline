...
TOKEN_DOCX_DIR = Path("output/tokens")

DATA_DICT_FILE = Path("data_dictionary.xlsx")
...
def load_data_dictionary(path: Path) -> Dict[str, str]:
    """
    Load mapping from bookmark → token from data_dictionary.xlsx.
    Expects columns: 'bookmark' and 'token'.
    """
    df = pd.read_excel(path)

    if "bookmark" not in df.columns or "token" not in df.columns:
        raise ValueError("data_dictionary.xlsx must have 'bookmark' and 'token' columns")

    mapping: Dict[str, str] = {}
    for _, row in df.iterrows():
        bookmark = str(row["bookmark"]).strip()
        token = row["token"]

        if pd.isna(bookmark) or pd.isna(token):
            continue

        token_str = str(token).strip()
        if not bookmark or not token_str:
            continue

        mapping[bookmark] = token_str
...
def apply_data_dict_to_doc(
    ...
    with their corresponding tokens from data_dictionary, and save to output/tokens.

    Returns (bookmarks_replaced, tokens_used).
    """
        ...
        for run in para.runs:
            ...
            for bookmark, token in bookmark_to_token.items():
                if bookmark in new_text:
                    ...
                    tokens_used_set.add(token)

            if new_text != text:
                run.text = new_text

    # Save updated DOCX into output/tokens
    out_path = TOKEN_DOCX_DIR / doc_path.name
    doc.save(out_path)

    return sorted(bookmarks_replaced_set), sorted(tokens_used_set)


def copy_doc_without_changes(doc_path: Path) -> None:
    """
    Ensure the doc exists in output/tokens even if we don't replace anything.
    """
    TOKEN_DOCX_DIR.mkdir(parents=True, exist_ok=True)
    out_path = TOKEN_DOCX_DIR / doc_path.name
...
def main() -> None:
    ...
    with TALLY_FILE.open("r", encoding="utf-8", newline="") as f_in:
        ...
        # - tokens: tokens actually inserted into this doc
        out_fieldnames = fieldnames + ["bookmarks_replaced", "tokens"]
        ...
        with FINAL_TALLY_FILE.open("w", encoding="utf-8", newline="") as f_out:
            ...
            for row in reader:
                ...
                if docx_path.exists():
                    bookmarks_replaced, tokens_used = apply_data_dict_to_doc(
