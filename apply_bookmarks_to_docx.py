...
DATA_DICT_FILE = Path("data_dictionary.xlsx")  # NEW
...
def load_valid_bookmarks_from_excel(path: Path) -> set[str]:
    """
    Load valid bookmark names from data_dictionary.xlsx.
    Expects columns: 'bookmark' and 'token'.
    Only the 'bookmark' values are used as a filter here.
    """
    df = pd.read_excel(path)
    # Ensure we have the required column
    if "bookmark" not in df.columns:
        raise ValueError("data_dictionary.xlsx must have a 'bookmark' column")

    # Build a set of normalized bookmark names as strings
    bookmarks = (
        df["bookmark"]
        .dropna()
        .astype(str)
...
def write_unique_bookmarks(unique_set: set):
    ...
    with open(UNIQUE_FILE, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["bookmark_name", "token"])
